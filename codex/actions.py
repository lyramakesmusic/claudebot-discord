"""Bot action dispatcher for codex bot."""
...
async def execute_bot_actions(
    ...
    for act in actions:
        ...
        if action == "upload":
            ...
            if not file_path:
                results.append("(upload skipped â€” no path given)")
                continue
            p = Path(file_path).expanduser()
            if not p.exists():
                results.append(f"Upload failed â€” file not found: `{file_path}`")
                continue
            size_mb = p.stat().st_size / (1024 * 1024)
            if size_mb > 500:
                results.append(f"Upload failed â€” file too large ({size_mb:.1f} MB, max 500 MB)")
                continue
            try:
